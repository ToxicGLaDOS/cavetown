...
@export var inventory_icon_scene: PackedScene
@export var selector_indicator: NodePath

var inventory_size: int = 10
var inventory: Array = []

var selector_node: Node
var selected_index: int: get = get_selected_index
var selected_item: ItemStack: get = get_selected_item
...
func _input(event):
	if event is InputEventKey:
		...
		if key_event.keycode == KEY_RIGHT and key_event.is_pressed():
			var sibling_index = get_selected_index()
		...
		elif key_event.keycode == KEY_LEFT and key_event.is_pressed():
			var sibling_index = get_selected_index()
			var new_parent = get_child(fposmod(sibling_index - 1, get_child_count()) as int)
			selector_node.get_parent().remove_child(selector_node)
			new_parent.add_child(selector_node)
		elif key_event.keycode == KEY_ENTER and key_event.is_pressed():
			var sibling_index = get_selected_index()
...
func add_item(item_stack):
	...
	if inventory[get_selected_index()] != null:
		...
		var item_icon = inventory_icon_scene.instantiate()
		var icon_texture = item_icon.get_node("TextureRect")
		icon_texture.texture = item_stack.item.inventory_texture
		inventory[get_selected_index()] = item_stack
		child.add_child(item_icon)
		child.get_node("InventoryIcon").get_node("Label").set_text(str(item_stack.quantity))
