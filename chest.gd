...
export(NodePath) var chest_ui_path
...
func _ready():
    ...
    chest_ui.connect("item_selected", self, "_withdraw_item_stack")
...
func _on_menu_closed(inventory: Inventory):
    inventory.disconnect('item_selected', self, '_add_item_stack')
...
func player_activate(p: Player):
    ...
    var err = inventory.connect('item_selected', self, '_add_item_stack')
    if err != OK:
        push_error("Failed to connect signal item_selected. Error was %s." % err)

    connect('_menu_closed', self, '_on_menu_closed', [inventory], CONNECT_ONESHOT)
